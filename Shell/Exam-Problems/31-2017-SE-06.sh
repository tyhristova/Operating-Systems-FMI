...
root_rss=$(ps -u root -o rss= | awk 'BEGIN {sum = 0} { sum += $1 } END { print sum }')
...
while IFS=: read -r user _ uid _ _ home _; do
    ...
    if [[ "${invalid}" -eq 1 ]]; then
        user_rss=$(ps -u "${user}" -o rss= 2>/dev/null | awk 'BEGIN {sum = 0} { sum += $1 } END { print sum }')

        if [[ "${user_rss}" -gt "${root_rss}" ]]; then
            pids=$(ps -u "${user}" -o pid=)
            if [[ -n "${pids}" ]]; then
                echo "Killing processes of user ${user}"
                kill ${pids}
            fi
        fi
    fi
...
# ${perms:2:1} -> взима substring от 2-ра позиция 1 символ (drwxr-xr-x)