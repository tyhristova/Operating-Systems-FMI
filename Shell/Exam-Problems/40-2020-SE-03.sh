...
if [[ ! -d "$PACKAGE" || ! -f "$PACKAGE/version" || ! -d "$PACKAGE/tree" ]]; then
...
tmp_tar="${REPO}/packages/package.tar"
tmp_xz="${REPO}/packages/package.tar.xz"

tar -cf "$tmp_tar" -C "$PACKAGE/tree" .
xz -f "$tmp_tar"

hash="$(sha256sum "$tmp_xz" | cut -d ' ' -f 1)"

old_hash="$(grep -E "^${package_id} " "$REPO/db" | cut -d ' ' -f 2)"

if [[ -n "$old_hash" ]]; then
    rm -f "${REPO}/packages/${old_hash}.tar.xz"
fi

mv "$tmp_xz" "${REPO}/packages/${hash}.tar.xz"

grep -Ev "^${package_id} " "$REPO/db" > "$REPO/db.tmp"
echo "${package_id} ${hash}" >> "$REPO/db.tmp"
sort "$REPO/db.tmp" > "$REPO/db"
rm -f "$REPO/db.tmp"
