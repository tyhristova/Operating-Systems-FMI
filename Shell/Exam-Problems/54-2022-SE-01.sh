...
Ако във файла има ред за устройство, което не съществува, скриптът да извежда предупреждение. Обърнете внимание на обработката за грешки и съобщенията към потребителя – искаме скриптът да бъде удобен и валиден инструмент.
...
if [[ "{#}" -ne 1 ]]; then 
    echo "Invalid amount of arguments!" 1>&2
    exit 1
fi

if [[ ! -f "$1" && ! -r "$1" ]]; then 
    echo "The argument must be a readable file!" 1>&2
    exit 2
fi

grep -v '^[[:space:]]*#' "$1" | sed 's/#.*//' | awk 'NR > 0 { print $1, $2 }' | while read -r device desired; do 
    current=$(awk -v dev=$device '$1 == dev { print $3 }' $WAKEUP | tr -d '*' )

    if [[ -z "$current" ]]; then 
        echo "Warning: device '$device' not found in $WAKEUP" 1>&2
        continue
    fi
    
    if [ "$desired" != "enabled" ] && [ "$desired" != "disabled" ]; then
        echo "Warning: invalid state '$desired' for device '$device'" 1>&2
        continue
    fi

    if [ "$current" != "$desired" ]; then
        echo "Toggling $device: $current -> $desired"
        echo "$device" > $WAKEUP
    else
        echo "$device already $desired, skipping"
    fi
done
